...
let jFLocalToInputTableFooterCategoryId = ({ inValue }) => {
    let targetButton = Array.from(document.querySelectorAll('.CategoryItems')).find(button =>
        button.textContent.trim() === inValue
    );

    if (targetButton) {
        targetButton.click(); // Simulate the click event
    };
};
